...
const int CONNECTIONS[][2] = {
    {136, 135}, {135, 214}, {214, 216}, {216, 206}, {206, 203}, {203,  98}, { 98,  97}, { 97,   2}, {  2, 326}, {326, 327},
    {327, 423}, {423, 426}, {426, 436}, {436, 434}, {434, 364}, {364, 365}, {365, 379}, {379, 378}, {378, 400}, {400, 377},
    {377, 152}, {152, 148}, {148, 176}, {176, 149}, {149, 150}, {150, 136}
};
...
int main(int argc, char **argv) {
    ...
    std::string path = std::string(argv[1]) + "/mediapipe/modules/face_landmark/face_landmark_front_cpu.binarypb";  // model path 
    mp_instance_builder *builder = mp_create_instance_builder(path.c_str(), "image"); 

    // Configure the graph with node options and side packets. packet
    mp_add_side_packet(builder, "num_faces", mp_create_packet_int(1)); 
    mp_add_side_packet(builder, "with_attention", mp_create_packet_bool(true)); 
    ...
    mp_poller *landmarks_poller = mp_create_poller(instance, "multi_face_landmarks");
    CHECK_MP_RESULT(landmarks_poller)

    // Create a poller for the hand rectangles.
    mp_poller *rects_poller = mp_create_poller(instance, "face_rects_from_detections");
